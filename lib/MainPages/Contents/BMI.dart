...
class _BMICalculatorPageState extends State<BMICalculatorPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (authState is AuthAuthenticated && authState.user != null) {
      ...
      if (now.month < user.birthMonth ||
          (now.month == user.birthMonth && now.day < user.birthDay)) {
        userAge--;
      }
    }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              const Text("We will use your physical information from your profile.",
                  style: TextStyle(
                      color: Colors.grey)),
              ...
              Row(
                children: [
                  Expanded(child: _buildInfoCard(
                      "Height", "${userHeight.toStringAsFixed(0)} cm",
                      Icons.height)),
                  const SizedBox(width: 10),
                  Expanded(child: _buildInfoCard(
                      "Weight", "$userWeight kg", Icons.monitor_weight)),
                ],
              ),
              const SizedBox(height: 10),
              Row(
                children: [
                  Expanded(child: _buildInfoCard(
                      "Age", "$userAge", Icons.cake)),
                  const SizedBox(width: 10),
                  Expanded(child: _buildInfoCard(
                      "Gender", userGender, Icons.person)),
                ],
              ),
              ...
              Center(
                child: TextButton.icon(
                  ...
                  style: TextButton.styleFrom(
                      foregroundColor: AppThemeColors.icons_color),
                ),
              ),
              ...
              BlocConsumer<BmiBloc, BmiState>(
                ...
                builder: (context, state) {
                  ...
                  return ElevatedButton(
                      ...
                      if (userHeight > 0 && userWeight > 0 && userAge > 0) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text(
                              "Please update your profile with valid Height, Weight and Birthdate.")),
                        );
                      }
                    },
                    ...
                    child: const Text("CALCULATE & UPDATE BMI",
              ...
              BlocBuilder<BmiBloc, BmiState>(
                builder: (context, state) {
                  if (state is BmiSuccess && state.latestBmi != null) {
                    ...
                    return Column(
                      children: [
                        const Text("Your BMI Result:", style: TextStyle(fontSize: 22,
                            fontWeight: FontWeight.bold,
                            color: Colors.black87)),
                        const SizedBox(height: 10),
                        Text(
                        ...
                        const SizedBox(height: 10),
                        Text("Last calculated: ${result.calculationDate
                            .toString().split(' ')[0]}", style: const TextStyle(
                            color: Colors.grey)),
                      ],
                    );
                  }
  ...
  Color _getColorForStatus(BmiStatus status) {
    switch (status) {
      ...
      case BmiStatus.normalWeight:
        return AppThemeColors.primary_color;
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text("BMI Categories", style: TextStyle(fontSize: 18,
                fontWeight: FontWeight.bold,
                color: Colors.black87)),
            const SizedBox(height: 10),
            Table(
              ...
              children: [
                ...
                _buildTableRow(
                    "25.0 - 29.9", "Overweight", color: Colors.orange),
                _buildTableRow("30.0 - 34.9", "Obesity Class I",
                    color: Colors.red.shade400),
                _buildTableRow("35.0 - 39.9", "Obesity Class II",
                    color: Colors.red.shade600),
                _buildTableRow(
                    ">= 40.0", "Obesity Class III", color: Colors.red.shade800),
              ],
            ),
          ],
        ),
  ...
  TableRow _buildTableRow(String range, String status,
